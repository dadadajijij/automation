...
- 当前还会额外记录：
  ...
  - 若存在显式声明，则记录 `artifacts.subpath_declaration`
...
1. 若 `.ka/subpath.json` 存在且内容有效，则优先生成 `subpath_plan`
2. 若声明不存在、JSON 非法、或 `projects` 全部无效，则回退自动识别
...
- 文件不存在：不影响部署，继续自动识别
- JSON 非法：当前实现忽略声明，继续自动识别
- `projects` 无有效项目：当前实现忽略声明，继续自动识别
- 只要能成功构建出至少一个声明式 project strategy，就优先信声明
...
- 若 finding 命中当前支持的 rewrite rule / auto-fix 能力，会触发自动修复；补救后的再次审计会追加到 `artifacts.subpath_static_audit_attempts`
...
### 4.4.5 `subpath_rewrite_report` 输出格式

当前 rewrite 阶段除了 `artifacts.rewritten_frontend_files` 之外，还会固定输出：
...
- key：repo 相对文件路径
- value：实际命中的 rewrite rule id 列表
...
10. 如果改写结果与预期不符，先看：
   ...
   - `artifacts.subpath_plan`
   - `artifacts.subpath_rewrite_report`
11. 如果主状态成功但访问异常，再单独看 `artifacts.athena_nginx_sync`
