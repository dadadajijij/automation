...
如果你需要的是平台内部执行规范，而不是架构说明，优先看：

- [SUBPATH_POLICY.md](./SUBPATH_POLICY.md)
...
1. 若 `.ka/subpath.json` 存在且能成功产出至少一个 project strategy，则优先使用声明式配置
2. 若声明不存在、JSON 非法、或 `projects` 全部无效，再回退自动识别
3. 自动识别仍不明确时，退化到 `generic + strip_prefix + static_rewrite`

当前实现不会做“声明一部分、剩下继续自动猜”的隐式 merge；声明一旦有效，会优先生成整份内部 `SubpathPlan`。
...
- `adapter` 缺省时：
  ...
  - 其他类型默认 `static_rewrite`
- `source_adapter` 缺省时：
  - `nextjs` 默认 `nextjs`
  - 其他类型默认 `static_rewrite`
- 不存在的路径字段会被忽略，不会中断整体流程
...
### 4.6 `SubpathPlan` / `SubpathStrategy` / declaration

当前内部流程已经不是只维护 repo 级 `SubpathStrategy`。
...
然后再映射回兼容型 `SubpathStrategy`，供旧调用方使用。
...
当前改写阶段已经不只记录“哪些文件被改”，还会记录“哪些 rewrite rule 命中了这些文件”。
...
- `auto_fix_findings()` 当前已经会参考 rewrite rule 的 `finding_codes` 元数据，而不只是框架 if/else
...
- 优先 browser runtime audit 入口
- 不可用时回退 HTML runtime audit

因此未来接入真实浏览器实现时，不需要再改上层主流程接口。
